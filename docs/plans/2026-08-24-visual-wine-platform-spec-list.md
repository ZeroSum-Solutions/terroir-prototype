...
| 0155 | `effective_service_pour_events_service_role_grant.sql` | Availability-only ACL bridge: grants `service_role` SELECT on the sealed 0153 `security_invoker` effective-event view so approved server readers can use the undo-correct projection with mandatory explicit restaurant predicates. It exact-matches and preserves either the clean authenticated-SELECT-only profile or the Supabase authenticated five-privilege profile, rejects all other tuples/grant options/DML drift, and requires one caller-owned transaction for the ACL and migration-ledger mutation; the paired down restores the same admitted profile exactly. No raw-event fallback, RPC, view replacement, authority expansion, or C04/C06 completion claim. | 0074; 0153 | TER-CF-305 (availability correction only; no C04/C06 completion claim) |
