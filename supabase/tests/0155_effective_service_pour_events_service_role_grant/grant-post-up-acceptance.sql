...
begin
  ...
  if not exists (
    ...
    raise exception 'C06_0155_EFFECTIVE_VIEW_SEAL_MISMATCH';
  end if;
  ...
  if coalesce(cardinality(v_acl_profiles), 0) <> 1 then
    raise exception 'C06_0155_EXACT_ACL_TUPLE_MISMATCH';
  end if;

  raise notice 'C06_0155_POST_UP_ACL_PROFILE=%', v_acl_profiles[1];
end;
$acceptance$;

\echo C06_0155_POST_UP_ACCEPTANCE_PASS
