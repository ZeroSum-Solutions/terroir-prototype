-- Reverse 0153 only while every row remains exactly representable by the
-- legacy version-1 reusable-slot schema. Run in one explicit transaction.
...
begin
  ...
  if v_table_count <> 6 then
    raise exception 'unsafe_down_physical_bottle_catalog_drift' using errcode = 'P0001';
  end if;
  ...
  if to_regprocedure('public.current_inventory_contract_version()') is null
     ...
     or to_regprocedure('public.list_open_bottle_aggregates(uuid)') is null
     or to_regclass('public.effective_service_pour_events') is null
     or to_regclass('public.bottle_closeouts_open_bottle_tenant_wine_idx') is null
     or to_regclass('public.pour_events_open_bottle_tenant_wine_idx') is null
     or to_regclass('public.pour_events_operation_entry_key') is null
     or to_regclass('public.pour_events_reversal_key') is null
     or to_regclass('public.inventory_command_bottle_effects_open_operation_key') is null
     or to_regclass('public.inventory_command_bottle_effects_bottle_idx') is null
     or to_regclass('public.inventory_command_bottle_effects_wine_idx') is null
     or not exists (
       select 1
     ...
     -- C06_PHASE_A_DOWN_SEALED_CONSTRAINT_GUARD_BEGIN
     or (
       ...
       select array_agg(name collate "C" order by name collate "C")
         from unnest(array[
           -- C06_PHASE_A_DOWN_EXPECTED_SEALED_CONSTRAINTS_BEGIN
           ...
           -- C06_PHASE_A_DOWN_EXPECTED_SEALED_CONSTRAINTS_END
         ]) name
     )
     or exists (
       select 1
     ...
     -- C06_PHASE_A_DOWN_SEALED_CONSTRAINT_GUARD_END
     or exists (
       select 1
         from pg_catalog.pg_class c
        where c.oid in (
     ...
     or not has_function_privilege('authenticated', 'public.current_inventory_contract_version()', 'EXECUTE')
     ...
     or has_function_privilege('anon', 'public.execute_physical_bottle_command(uuid,uuid,text,uuid,uuid,uuid,integer,text,text,integer,integer,uuid,uuid,text,boolean)', 'EXECUTE')
     or has_function_privilege('authenticated', 'public.execute_physical_bottle_command(uuid,uuid,text,uuid,uuid,uuid,integer,text,text,integer,integer,uuid,uuid,text,boolean)', 'EXECUTE')
     or has_function_privilege('service_role', 'public.execute_physical_bottle_command(uuid,uuid,text,uuid,uuid,uuid,integer,text,text,integer,integer,uuid,uuid,text,boolean)', 'EXECUTE')
     or has_function_privilege('anon', 'public.execute_physical_reconciliation_batch(uuid,uuid,jsonb)', 'EXECUTE')
     or has_function_privilege('authenticated', 'public.execute_physical_reconciliation_batch(uuid,uuid,jsonb)', 'EXECUTE')
     or has_function_privilege('service_role', 'public.execute_physical_reconciliation_batch(uuid,uuid,jsonb)', 'EXECUTE')
     or exists (
       select 1
         from pg_catalog.pg_class c,
              pg_catalog.aclexplode(coalesce(c.relacl, pg_catalog.acldefault('r', c.relowner))) acl
     ...
     or not has_table_privilege('authenticated', 'public.effective_service_pour_events', 'SELECT')
     or exists (
       select 1
  ...
  if exists (
    select 1
     ...
     or (select count(*) from pg_catalog.pg_policy
          where polrelid = 'public.inventory_command_bottle_effects'::regclass) <> 0
     or (select count(*) from pg_catalog.pg_policy
  ...
  if exists (select 1 from public.inventory_command_bottle_effects limit 1)
     ...
     or exists (
       select 1 from public.bottle_closeouts where event_contract <> 1
     )
     or exists (
...
drop view public.effective_service_pour_events;
drop function public.list_open_bottle_aggregates(uuid);
drop function public.list_active_physical_bottles(uuid);
...
drop function public.current_inventory_contract_version();

drop table public.inventory_command_bottle_effects;

drop index public.bottle_closeouts_open_bottle_tenant_wine_idx;
alter table public.bottle_closeouts
  drop constraint bottle_closeouts_physical_shape_check,
  drop constraint bottle_closeouts_open_bottle_tenant_wine_fkey,
  drop constraint bottle_closeouts_event_contract_check,
  drop column event_contract,
  add constraint bottle_closeouts_open_bottle_id_fkey
    foreign key (open_bottle_id) references public.open_bottles(id) on delete set null;

drop index public.pour_events_reversal_key;
drop index public.pour_events_open_bottle_tenant_wine_idx;
drop index public.pour_events_operation_entry_key;
alter table public.pour_events
  drop constraint pour_events_physical_shape_check,
  drop constraint pour_events_reversal_of_event_fkey,
  drop constraint pour_events_operation_receipt_fkey,
  drop constraint pour_events_open_bottle_tenant_wine_fkey,
  drop constraint pour_events_operation_entry_ordinal_check,
  drop constraint pour_events_event_contract_check,
  drop constraint pour_events_kind_check,
  drop column reversal_of_event_id,
  drop column operation_entry_ordinal,
  drop column operation_id,
  drop column event_contract,
  add constraint pour_events_kind_check check (
    kind in ('pour', 'spill', 'reconcile', 'new_bottle', 'finish_bottle')
  ),
...
alter table public.inventory_command_receipts
  ...
  drop constraint inventory_command_receipts_batch_entry_count_check,
  drop constraint inventory_command_receipts_scope_kind_check,
  ...
  drop column batch_entry_count,
  drop column scope_kind,
  drop column command_version,
  alter column wine_id set not null,
  add constraint inventory_command_receipts_command_type_check check (
    command_type in ('open', 'pour', 'spill', 'discard', 'close')
  );

alter table public.open_bottles
  drop constraint open_bottles_physical_shape_check,
  drop constraint open_bottles_state_version_check,
  drop constraint open_bottles_source_provenance_check,
  drop constraint open_bottles_nominal_capacity_check,
  drop constraint open_bottles_identity_origin_check,
  drop constraint open_bottles_identity_contract_check,
  drop constraint open_bottles_id_restaurant_wine_key,
  drop column state_version,
  drop column opening_operation_id,
  drop column source_provenance,
  drop column nominal_capacity_ml,
  drop column identity_origin,
  drop column identity_contract;

alter table public.inventory_items
  drop constraint inventory_items_id_restaurant_wine_key;
