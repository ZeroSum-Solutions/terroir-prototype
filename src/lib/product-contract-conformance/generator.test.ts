...
function fixture() {
  ...
  const items = Array.from({ length: 317 }, (_, index) => {
...
describe("product contract conformance generator", () => {
  ...
  it("preserves the stable active ledger IDs", () => {
    ...
    expect(generated.scope.activeFeatureLedgerCount).toBe(317);
    ...
    expect(result.stderr).toContain("active feature ledger IDs must remain TER-CF-001 through TER-CF-317");
  });
